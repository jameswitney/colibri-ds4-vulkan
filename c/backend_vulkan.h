...
/* M5-1b: dsv4 batched expert-group phases — ONE submit per phase (vs 3 per
 * expert in M5-1a's per-op submits). The dsv4 chain needs the gate/up outputs
 * back on the host for its bitwise rounding chain (bf16/sigmoid/clamps/route
 * weight) between phases, so the fused GLM eg path (hidden stays on-device)
 * does not apply; instead every dispatch of a phase is recorded into ONE
 * command buffer, submitted once, fenced once:
 *   phase1: QDQ(x, block=128) + all experts' gate/up fmt=10 matmuls (all read
 *           the same QDQ'd activation) -> gate_out/up_out (count*I each).
 *   phase2: QDQ(hid rows, S=count — per-row independent, bitwise same as the
 *           CPU's per-expert QDQ) + all experts' down matmuls -> down_out
 *           (count*H each).
 * Convention (matches dsv4_cuda_expert_group): H = the activation dim (the
 * gate/up matmul INPUT, the tensor's I/columns), I = the moe intermediate dim
 * (the gate/up matmul OUTPUT, the tensor's O/rows); gate/up are H->I matmuls
 * and down I->H. count in [1,64]. Returns 0 on any failure/shape mismatch —
 * the caller falls back to the CPU chain. */
